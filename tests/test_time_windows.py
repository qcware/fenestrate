...
import pytz
import pytest
from fenestrate.fenestrate import Window, in_window, in_nonexcluded_window, next_window, ConcreteWindow
...
daily = Window(selectors.daily(), datetime.time(hour=11, tzinfo=pytz.timezone("US/Eastern")),
           datetime.time(hour=13, tzinfo=pytz.timezone("US/Eastern")))
weekly = Window(selectors.weekly_on(selectors.Weekday.WEDNESDAY), datetime.time(hour=12, tzinfo=pytz.timezone("US/Eastern")),
           datetime.time(hour=13, tzinfo=pytz.timezone("US/Eastern")))     
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ConcreteWindow(from_time=arrow.get("2020-06-24 11:00:00").replace(tzinfo="US/Eastern"),
